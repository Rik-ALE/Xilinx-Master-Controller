...
architecture rtl of timing is
  ...
  signal hpos_cnt : unsigned (10 downto 0) := (others => '0');
  signal hpos_clr : std_logic;
  signal hpos_ena : std_logic;
  signal vpos_cnt : unsigned (10 downto 0) := (others => '0');
...
begin
  ...
  hpos_ena <= '1';
  hpos_clr <= '1' when (( (hpos_cnt >= unsigned(tc_heblnk) ) and hpos_ena='1') or restart='1') else '0';

  vpos_ena <= hpos_clr;
  vpos_clr <= '1' when (( (vpos_cnt >= unsigned(tc_veblnk) ) and vpos_ena='1') or restart='1') else '0';
  ...
  hcount <= std_logic_vector(hpos_cnt);
  hblnk  <= '1' when  (hpos_cnt > unsigned(tc_hsblnk) ) else '0';
  hsync  <= '1' when ((hpos_cnt > unsigned(tc_hssync) ) and (hpos_cnt <= unsigned(tc_hesync) )) else '0';
  ...
  vcount <= std_logic_vector(vpos_cnt);
  vblnk  <= '1' when  (vpos_cnt > unsigned(tc_vsblnk) ) else '0';
  vsync  <= '1' when ((vpos_cnt > unsigned(tc_vssync) ) and (vpos_cnt <= unsigned(tc_vesync) )) else '0';
