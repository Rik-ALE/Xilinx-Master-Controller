...
architecture logic of Dram_Seq is
	...
	function To_Std_Logic(L: BOOLEAN) return std_ulogic is				-- boolean to lit
		begin
		if L then return('1');
		else return('0');
		end if;
...
--BUG429						if bExtraByte or bClearNoOff or (nAddrInRas>120/8) then		-- extra read should not use rasoff
						...
						TP(7) <= To_Std_Logic(bExtraByte);		-- 34mm: high for 1st read after 120 dot point (equals RD504)
						TP(6) <= To_Std_Logic(bClearNoOff);		-- 34mm: always low
						TP(5) <= To_Std_Logic(bDbg1);			-- hi heads 0 & 1
						TP(4) <= To_Std_Logic(bDbg2);			-- shows bad rasoff
  ...
  begin
	...
	HD34MM(0) <= '1' when HCONF16X8(14+16*0 downto 12+16*0) = "100" else '0';		-- true of each 504 head detected
